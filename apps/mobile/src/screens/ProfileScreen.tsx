    ...
    <ScrollView style={s.c} contentContainerStyle={{ paddingBottom: 40 }}
      ...
      <ImageBackground source={{ uri: IMG.grass }} style={s.heroBg} resizeMode="cover">
        <View style={s.heroOv}>
          ...
          {profile?.city && <View style={s.cityRow}><Ionicons name="location" size={12} color={C.t2} /><Text style={s.cityText}>{profile.city}</Text></View>}
        </View>
      </ImageBackground>
      ...
      <View style={s.body}>
          ...
          <View style={s.ratingCard}>
            ...
            <View style={s.ratingRight}>
              <View style={s.starsRow}>
                {[1,2,3,4,5].map(i => <Ionicons key={i} name={i <= Math.round(st.avgRating / 2) ? 'star' : 'star-outline'} size={16} color={C.gold} />)}
              </View>
        ...
        {profile?.bio && (
          <View style={s.bioCard}>
            <Ionicons name="chatbubble-outline" size={14} color={C.t3} style={{ marginRight: 8, marginTop: 2 }} />
        ...
        <View style={s.grid}>
          ...
          <Stat icon="git-merge-outline" color={C.blue} val={m.totalAssists || 0} label="Asistencias" />
          ...
          <StatCustom icon="card" color={C.orange} val={m.totalYellowCards || 0} label="Amarillas" />
...
function Stat({ icon, color, val, label }: { icon: any; color: string; val: number; label: string }) {
  return (
...
function StatCustom({ icon, color, val, label }: { icon: string; color: string; val: number; label: string }) {
  return (
    <View style={[gs.card, { borderLeftColor: color }]}>
...
const gs = StyleSheet.create({
  card: { backgroundColor: C.card, borderRadius: 12, padding: 14, alignItems: 'center', width: '31%', flexGrow: 1, borderWidth: 1, borderColor: C.border, borderLeftWidth: 3, gap: 4 },
...
const s = StyleSheet.create({
  ...
  heroBg: { height: 250 },
  heroOv: { flex: 1, backgroundColor: 'rgba(11,14,26,0.75)', alignItems: 'center', justifyContent: 'center', paddingTop: 20 },
  avatarRing: { width: 80, height: 80, borderRadius: 40, backgroundColor: C.surface, borderWidth: 2, borderColor: C.primary, justifyContent: 'center', alignItems: 'center', marginBottom: 12 },
  avatarLetter: { color: C.primary, fontSize: 30, fontWeight: '800' },
  nick: { color: C.w, fontSize: 22, fontWeight: '800' },
  posChip: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: C.primaryMuted, paddingHorizontal: 12, paddingVertical: 4, borderRadius: 16, marginTop: 8 },
  ...
  body: { padding: 18, marginTop: -16, borderTopLeftRadius: 20, borderTopRightRadius: 20, backgroundColor: C.bg },
  ratingCard: { flexDirection: 'row', backgroundColor: C.card, borderRadius: 14, padding: 18, alignItems: 'center', marginBottom: 14, borderWidth: 1, borderColor: C.border },
  ...
  bioCard: { flexDirection: 'row', backgroundColor: C.card, borderRadius: 12, padding: 14, marginBottom: 18, borderWidth: 1, borderColor: C.border },
  ...
  logoutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: C.redMuted, padding: 14, borderRadius: 12, borderWidth: 1, borderColor: 'rgba(255,71,87,0.25)' },
