...
export function MatchDetailScreen({ route, navigation }: any) {
  ...
  const handleComplete = async () => {
    const hs = parseInt(homeScoreInput),
      as_ = parseInt(awayScoreInput);
    if (isNaN(hs) || isNaN(as_) || hs < 0 || as_ < 0)
      return showAlert('Error', 'Marcadores inválidos');
    setCompleting(true);
    try {
      await matchApi.complete(matchId, hs, as_);
      showAlert('Finalizado', 'Ventana de votación abierta 12h');
      fetchMatch();
  ...
  if (!match)
    return (
      <View style={[s.c, s.ctr]}>
        <Text style={{ color: C.t2 }}>Partido no encontrado</Text>
      </View>
    );
  ...
  const votingOpen =
    isCompleted && match.votingDeadline && new Date(match.votingDeadline) > new Date();
    ...
    <ScrollView style={s.c} contentContainerStyle={s.cc}>
      ...
      <View style={s.infoRow}>
        <View style={[s.tag, { backgroundColor: gc.bg }]}>
          <Text style={[s.tagT, { color: gc.accent }]}>
            {match.gameType} · {gc.label}
          </Text>
        </View>
      </View>
      <View style={s.locRow}>
      ...
      <View style={s.board}>
        <View style={s.side}>
          <View style={s.shield}>
            <Ionicons name="shirt" size={28} color={C.t2} />
          </View>
        ...
        <View style={[s.side, { alignItems: 'flex-end' }]}>
          <View style={s.shield}>
            <Ionicons name="shirt-outline" size={28} color={C.t2} />
          </View>
            ...
            <View key={idx} style={s.section}>
              ...
              {team.players?.map((p: any) => (
                <View key={p.id} style={s.playerRow}>
                  <View style={s.playerAv}>
                    <Text style={s.playerAvT}>{(p.user?.nickname || 'J')[0]}</Text>
                  </View>
      ...
      {isPending && (
        <View style={s.invActions}>
          <TouchableOpacity
            style={[s.invBtn, { backgroundColor: C.primary }]}
            onPress={() => handleInvitation('ACCEPTED')}
          >
            <Ionicons name="checkmark" size={18} color={C.bg} />
            <Text style={s.invBtnT}>Aceptar</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[s.invBtn, { backgroundColor: C.red }]}
            onPress={() => handleInvitation('DECLINED')}
          >
            <Ionicons name="close" size={18} color={C.w} />
            <Text style={[s.invBtnT, { color: C.w }]}>Rechazar</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {isCreator &&
        ...
        !showScoreForm && (
          <TouchableOpacity style={s.completeBtn} onPress={() => setShowScoreForm(true)}>
            ...
            <Text style={s.completeBtnT}>Finalizar partido</Text>
          </TouchableOpacity>
        )}
        ...
        <View style={s.scoreForm}>
          ...
          <TouchableOpacity style={s.confirmBtn} onPress={handleComplete} disabled={completing}>
            <Text style={s.confirmBtnT}>
              {completing ? 'Finalizando...' : 'Confirmar resultado'}
            </Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {votingOpen && (
        <TouchableOpacity
          style={s.voteBtn}
          onPress={() => navigation.navigate('Voting', { matchId, match })}
        >
          <Ionicons name="thumbs-up" size={18} color={C.bg} />
          <Text style={s.voteBtnT}>Votar MVP y valoraciones</Text>
        </TouchableOpacity>
      )}
    </ScrollView>
  );
