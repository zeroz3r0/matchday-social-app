...
export function VotingScreen({ route, navigation }: any) {
  ...
  const handleSubmit = async () => {
    ...
    if (unrated.length > 0)
      return showAlert('Faltan valoraciones', `Valora a los ${unrated.length} jugadores restantes`);
        ...
        <View key={player.id} style={s.card}>
          <View style={s.cardHead}>
            <View style={s.cardLeft}>
              <View style={s.av}>
                <Text style={s.avT}>{player.nickname[0]}</Text>
              </View>
              <View>
                <Text style={s.pName}>{player.nickname}</Text>
                <Text style={s.pTeam}>{player.teamName}</Text>
              </View>
            </View>
            <TouchableOpacity
              style={[s.mvpBtn, mvpPick === player.id && s.mvpBtnOn]}
          ...
          <View style={s.ratingRow}>
            {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
              <TouchableOpacity
                key={n}
                style={[s.rBtn, (ratings[player.id] || 0) >= n && s.rBtnOn]}
                onPress={() => setRatings((prev) => ({ ...prev, [player.id]: n }))}
              >
