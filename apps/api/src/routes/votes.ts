  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (match.status !== 'COMPLETED')
        throw new AppError(400, 'MATCH_NOT_COMPLETED', 'Partido no finalizado');
      ...
      if (!voterPlayer) {
        throw new AppError(
          403,
          'NOT_PARTICIPANT',
          'Solo los participantes del partido pueden votar',
        );
      }
    ...
    try {
      ...
      if (userId !== match.createdById) {
        throw new AppError(
          403,
          'FORBIDDEN',
          'Solo el creador del partido puede cerrar la votacion',
        );
      }
